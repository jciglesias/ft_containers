...
class ft::bst_iterator : public ft::iterator< ft::random_access_iterator_tag, t_node<Key, T> >
{
public:
	typedef typename ft::iterator<ft::random_access_iterator_tag, t_node<Key, T> >	iterator;
	...
	bst_iterator(pointer ptr, pointer end) : _node(ptr), _end(end) {}
...
*/
	...
	pointer	end(void) const { return _end; }
/*
**operators
*/
...
private:
	pointer	_node;
	pointer	_end;
	pointer	search_behind(pointer n, pointer before){
		if (n->left != 0){
			if (n->left->right != 0)
		...
		else if (n->right != 0 && n->right->first < _node->first)
			n = search_behind(n->right, n);
		else if (n->up != 0 && n->up != before)
			return n->up;
		return n;
	}
	pointer	upper_bound(pointer node, pointer before){
		if (node == 0)
			return node;
		if (node != 0 && node->first > _node->first)
			return node;
